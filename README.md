...
`WebViewSDK` is an iOS SDK that allows a native iOS application to connect to a web app using `WKWebView`. It injects session and user data into the web layer, listens for messages sent back from the web app, and supports native notification handling. :contentReference[oaicite:0]{index=0}
...
- `InitializeSdkViewController` — Main SDK view controller that loads the web app in a `WKWebView` and handles communication with JavaScript :contentReference[oaicite:2]{index=2}
- `config` — Stores SDK configuration such as user ID, session ID, auth token, and account data :contentReference[oaicite:3]{index=3}
- `NotificationManager` — Handles local notification presentation and user interaction :contentReference[oaicite:4]{index=4}
